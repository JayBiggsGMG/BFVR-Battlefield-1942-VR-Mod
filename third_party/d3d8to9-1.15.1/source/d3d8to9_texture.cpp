...
HRESULT STDMETHODCALLTYPE Direct3DTexture8::GetLevelDesc(UINT Level, D3DSURFACE_DESC8 *pDesc)
{
	...
	if (EmulatesManagedPool)
	{
		pDesc->Pool = D3DPOOL_MANAGED;
		pDesc->Usage = EmulatedManagedUsage;
	}
...
HRESULT STDMETHODCALLTYPE Direct3DTexture8::GetSurfaceLevel(UINT Level, IDirect3DSurface8 **ppSurfaceLevel)
{
	...
	*ppSurfaceLevel = nullptr;
	...
	auto Surface = Device->ProxyAddressLookupTable->FindAddress<Direct3DSurface8>(SurfaceInterface);
	if (EmulatesManagedPool)
		Surface->SetEmulatedManagedPool(EmulatedManagedUsage);
...
HRESULT STDMETHODCALLTYPE Direct3DVolumeTexture8::GetVolumeLevel(UINT Level, IDirect3DVolume8 **ppVolumeLevel)
{
	...
	*ppVolumeLevel = nullptr;
	...
	auto Volume = Device->ProxyAddressLookupTable->FindAddress<Direct3DVolume8>(VolumeInterface);
	if (EmulatesManagedPool)
		Volume->SetEmulatedManagedPool(EmulatedManagedUsage);
