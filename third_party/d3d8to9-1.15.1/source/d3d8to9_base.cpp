...
HRESULT STDMETHODCALLTYPE Direct3D8::CreateDevice(UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags, D3DPRESENT_PARAMETERS8 *pPresentationParameters, IDirect3DDevice8 **ppReturnedDeviceInterface)
{
	...
	*ppReturnedDeviceInterface = nullptr;
	...
	bool IsExtendedDevice = false;

	HRESULT hr = E_FAIL;
	IDirect3D9Ex *ProxyInterfaceEx = nullptr;
	if (SUCCEEDED(ProxyInterface->QueryInterface(
			__uuidof(IDirect3D9Ex),
			reinterpret_cast<void **>(&ProxyInterfaceEx))) &&
		ProxyInterfaceEx != nullptr)
	{
		D3DDISPLAYMODEEX FullscreenMode = {};
		FullscreenMode.Size = sizeof(FullscreenMode);
		FullscreenMode.Width = PresentParams.BackBufferWidth;
		FullscreenMode.Height = PresentParams.BackBufferHeight;
		FullscreenMode.RefreshRate =
			PresentParams.FullScreen_RefreshRateInHz;
		FullscreenMode.Format = PresentParams.BackBufferFormat;
		FullscreenMode.ScanLineOrdering =
			D3DSCANLINEORDERING_PROGRESSIVE;

		IDirect3DDevice9Ex *DeviceInterfaceEx = nullptr;
		hr = ProxyInterfaceEx->CreateDeviceEx(
			Adapter,
			DeviceType,
			hFocusWindow,
			BehaviorFlags,
			&PresentParams,
			PresentParams.Windowed ? nullptr : &FullscreenMode,
			&DeviceInterfaceEx);
		DeviceInterface = DeviceInterfaceEx;
		IsExtendedDevice = DeviceInterfaceEx != nullptr;
		ProxyInterfaceEx->Release();
	}
	if (DeviceInterface == nullptr)
	{
		hr = ProxyInterface->CreateDevice(
			Adapter,
			DeviceType,
			hFocusWindow,
			BehaviorFlags,
